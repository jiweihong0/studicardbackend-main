...
export default (ctx: { [key: string]: any }, app: Express) => {
    ...
    router.post("/login", async (req, res) => {
        ...
        try {
            ...
            const info = { account_id: account.account_id };
    ...
    router.get("/get", verifyToken(), async (req, res) => {
        try {
            ...
            const account = await Account.findOne({
                where: {
                    account_id: res.locals.decoded.account_id,
                }
            })
            if (!account) {
                return res.status(404).json({ message: "無此用戶" });
            ...
            res.status(400).json({ message: "取得資料失敗，請重新再試一次", error: err });
        }
    })
    ...
    router.patch("/updatepassword", verifyToken(), async (req, res) => {
        ...
        try {
            ...
            return res.status(200).json({ message: "更改密碼成功" });
        } catch (err) {
            res.status(400).json({ message: "更改失敗，請重新再試一次", error: err });
        }
    })
