...
const isDecodedAccountInDatabase = async (account_id:string): Promise<boolean> => {
...
export default () => {
    return async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            console.log(decoded,111111111)
            const isdecodedAccountInDatabase = await isDecodedAccountInDatabase(decoded.account_id)
